...
#include "../../SyscallMon/syscall_mon.h"
...
_Use_decl_annotations_ NTSTATUS VmInitialization() {
  ...
  //status = SyscallmonInitialization(shared_data->shared_sh_data);
  status = ShadowHookInitialization(shared_data->shared_sh_data);
  if (!NT_SUCCESS(status)) {
    UtilForEachProcessor(VmpStopVm, nullptr);
    return status;
  }
...
_Use_decl_annotations_ bool VmpIsHyperPlatformInstalled() {
  PAGED_CODE();
