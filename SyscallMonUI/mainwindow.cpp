...
void MainWindow::InitEventView(void)
{
    ...
    ui->checkBox_DropExclude->setCheckState(
                m_EventMgr->m_DropExclude ?
    ...
    ui->tableView_Event->setColumnWidth(3, 75);
    ui->tableView_Event->setColumnWidth(4, 100);
    ui->tableView_Event->setColumnWidth(5, 300);
    ui->tableView_Event->setColumnWidth(6, 200);
    ui->tableView_Event->setColumnWidth(7, 200);
...
void MainWindow::on_checkBox_DropExclude_clicked(bool checked)
{
    ...
    m_EventMgr->SetConfig("Drop", checked ? "1" : "0");
}
...
{
    QString path = QFileDialog::getOpenFileName(this, tr("Open Dump"), ".", tr("Text Files(*.txt)"));
    m_EventMgr->DumpToFile(path);
}
