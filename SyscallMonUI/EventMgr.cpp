...
CEventMgr::CEventMgr(QObject *parent) : QObject(parent)
{
    ...
    QString code = setting.value("Script").toString();
    if (code != "")
    {
        SetLuaFilter(code);
    }
    code = setting.value("Drop").toString();
    if (code == "1")
    {
        this->m_DropExclude = TRUE;
    }
}
...
void CEventMgr::SetLuaFilter(const QString &code, bool save)
{
    lua_State *LL = (lua_State *)L;

    int ok = luaL_loadstring(LL, code.toStdString().c_str());
    if (LUA_OK != ok)
    {
        const char *errstring = lua_tostring(LL, -1);
        QMessageBox::information(NULL, "ERROR", errstring, QMessageBox::Yes | QMessageBox::No, QMessageBox::Yes);
        return;
    }
    lua_setglobal(LL, "filter");
    filter_by_lua = true;
    ...
    {
        QSettings setting("config.ini", QSettings::IniFormat);
        setting.setValue("Script", code);
    }
}

void CEventMgr::SetConfig(const QString &Key, const QString &Val)
{
    QSettings setting("config.ini", QSettings::IniFormat);
...
bool CEventMgr::DoFilter(const CUniqueEvent *ev)
{
    if (filter_by_lua)
    {
        ...
        if (top > 10)
            abort();
        lua_getglobal(LL, "filter");

        const char * upname = lua_getupvalue(LL, -1, 1);
        if (upname == NULL)
        {
            lua_pop(LL, 1);
            return false;
        }
        lua_createtable(LL, 0, 0);

        ULONG pid = ev->GetProcessId();
        ULONG tid = ev->GetThreadId();

        lua_pushinteger(LL, pid);
        lua_setfield(LL, -2, "pid");
        lua_pushinteger(LL, tid);
        lua_setfield(LL, -2, "tid");
        lua_pushstring(LL, ev->GetEventName().toStdString().c_str());
        lua_setfield(LL, -2, "behav");
        //lua_pushstring(LL, ev->GetFullArgument().toStdString().c_str());
        //lua_setfield(LL, -2, "args");

        lua_setfield(LL, -2, "ev");
        lua_pop(LL, 1); //lua_getupvalue;

        if (LUA_OK != (st = lua_pcall(LL, 0, 1, 0)))
        {
            //const char *errmsg = lua_tostring(LL, -1);
            lua_pop(LL, 1);
            return false;
        };
        int r = lua_toboolean(LL, -1);
        lua_pop(LL, 1);
        return r > 0;
    }
    else
...
void CEventMgr::DumpToFile(const QString &path)
{
    Lock();
    //
    //this->OnCallStack()
    QFile SavedFile(path);
    if (!SavedFile.open(QIODevice::ReadWrite | QIODevice::Text))
    {
        Unlock();
        return;
    }
    QTextStream outstream(&SavedFile);
    ...
    for (auto it = m_EventList.begin(); it != m_EventList.end(); it++)
    {
        ...
        QString str1 = date.toString("HH:mm:ss");
        QString str2;
        QString str3;

        str1.append(QString::asprintf(" %03d", ev->GetEventTime() % 1000));
        ev->GetBriefArgument(str2);
        ev->GetBriefResult(str3);

        QString txt = str1 + "\t" + ev->GetDisplayName() + "\t" + QString::number(ev->GetProcessId()) + "\t"
             + QString::number(ev->GetThreadId()) + "\t" + ev->GetEventName() + "\t"
            + ev->GetEventPath() + "\t" + str2 + "\t" + str3 + "\n";
