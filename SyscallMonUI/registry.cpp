    ...
    : CRegKeyValue(Type, Size, szValueName)
{
    ...
    PWCH raw = (PWCH)BinaryData.constData();
    ...
    : CRegKeyValue(Type, Size, szValueName)
{
    ...
    if(BinaryData.size() >= sizeof(ULONG64)) {
        memcpy(&NumberData, BinaryData.constData(), sizeof(ULONG64));
    } else if(BinaryData.size() >= sizeof(DWORD)) {
        if(Type == REG_DWORD){
            memcpy(&NumberData, BinaryData.constData(), sizeof(DWORD));
        }else{
            ULONG temp;
            memcpy(&temp, BinaryData.constData(), sizeof(DWORD));
