...
CEventFilter *GetNewFilter(filter_key key, filter_rel rel, int valData, QString &valStr, bool bInclude)
{
    ...
    switch (key)
    {
        case FltKey_PID:
        {
            if (ValidateRelation(rel, FltRelTable_Number, _ARRAYSIZE(FltRelTable_Number)))
            {
                ...
                if (ok)
                {
                    pFilter = new CEventFilter_PID(ProcessId, rel, bInclude);
                }
            }
            break;
        }
        case FltKey_ProcessName:
        {
            if (!valStr.isEmpty() && ValidateRelation(rel, FltRelTable_String, _ARRAYSIZE(FltRelTable_String)))
                pFilter = new CEventFilter_ProcessName(valStr, rel, bInclude);
            break;
        }
        case FltKey_ProcessPath:
        {
            if (!valStr.isEmpty() && ValidateRelation(rel, FltRelTable_String, _ARRAYSIZE(FltRelTable_String)))
                pFilter = new CEventFilter_ProcessPath(valStr, rel, bInclude);
            break;
        }
        case FltKey_EventPath:
        {
            if (!valStr.isEmpty() && ValidateRelation(rel, FltRelTable_String, _ARRAYSIZE(FltRelTable_String)))
                pFilter = new CEventFilter_EventPath(valStr, rel, bInclude);
            break;
        }
        case FltKey_Arch:
        {
            ...
            {
                pFilter = new CEventFilter_Arch((valData == 1) ? true : false, rel, bInclude);
            }
            break;
        }
        case FltKey_SessionId:
        {
            if (ValidateRelation(rel, FltRelTable_Number, _ARRAYSIZE(FltRelTable_Number)))
            {
                ...
                if (ok)
                {
                    pFilter = new CEventFilter_SessionId(SessionId, rel, bInclude);
                }
            }
        ...
        case FltKey_EventType:
        {
            if (ValidateRelation(rel, FltRelTable_Binary, _ARRAYSIZE(FltRelTable_Binary)))
            {
                ...
                {
                    pFilter = new CEventFilter_EventType((EventType_t)valData, rel, bInclude);
                }
            }
            break;
        }
        case FltKey_EventClass:
        {
            if (ValidateRelation(rel, FltRelTable_Binary, _ARRAYSIZE(FltRelTable_Binary)))
            {
                ...
                {
                    pFilter = new CEventFilter_EventClass((EventClass_t)valData, rel, bInclude);
                }
            }
            break;
        }
        ...
        {
            if (!valStr.isEmpty() && ValidateRelation(rel, FltRelTable_String, _ARRAYSIZE(FltRelTable_String)))
                pFilter = new CEventFilter_BriefResult(valStr, rel, bInclude);
            break;
        }
    }
    return pFilter;
...
void FilterDialog::on_pushButton_add_clicked()
{
    ...
    CEventFilter *pFilter = GetNewFilter(key, rel, valData, valStr, bInclude);

    if (pFilter)
    {
        //No need to reference the flt since it's refcount = 1 when construct
        m_model_Filter->appendRow(pFilter);
        m_bModified = true;
...
void FilterDialog::on_pushButton_remove_clicked()
{
    ...
    {
        CEventFilter *pFilter = m_LocalFilterList.at(index.row());
...
void FilterDialog::on_pushButton_apply_clicked()
{
    ...
    //Copy local list to EventMgr's list, no need to lock here
    //m_EventMgr->Lock();
    m_EventMgr->LoadFilters(m_LocalFilterList);
    //m_EventMgr->Unlock();
...
void FilterDialog::showEvent(QShowEvent *e)
{
    ...
    m_model_Filter->setRows(list);
}

void FilterDialog::on_pushButtonApplyLua_clicked()
{
    QString code = ui->edit_luacode->toPlainText();
    m_EventMgr->SetLuaFilter(code, true);
}
