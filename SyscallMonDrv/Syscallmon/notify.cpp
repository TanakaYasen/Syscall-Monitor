...
PVOID MakePsCreateProcessEvent(ULONG64 EventId, HANDLE ParentId, HANDLE ProcessId, BOOLEAN Create)
{
	...
	if (data)
	{
		...
		data->Create = Create;
		data->ProcessId = (ULONG)(UINT_PTR)ProcessId;
		data->ThreadId = (ULONG)(UINT_PTR)PsGetCurrentThreadId();
		data->ParentProcessId = (ULONG)(UINT_PTR)ParentId;
	...
	if (m_EventList->IsCapturing())
	{
		...
		if (EventId)
		{
			...
			if (data)
			{
				...
				data->Create = Create;
				data->ProcessId = (ULONG)(UINT_PTR)ProcessId;
				data->ThreadId = (ULONG)(UINT_PTR)ThreadId;
				data->CurProcessId = (ULONG)(UINT_PTR)PsGetCurrentProcessId();
				data->CurThreadId = (ULONG)(UINT_PTR)PsGetCurrentThreadId();
...
VOID EnumSystemModules(VOID)
{
	...
	for (ULONG i = 0; i < pInfo->NumberOfModules; ++i)
	{
		...
		strModulePath.Length = (USHORT)strlen((const char *)pEntry->FullPathName);
	...
	if (m_EventList->IsCapturing() && pOperationInformation->Object != NULL)
	{
		...
		if (EventId)
		{
			...
			if (data)
			{
				...
				data->ThreadId = (ULONG)(UINT_PTR)PsGetCurrentThreadId();
				data->TargetProcessId = (ULONG)(UINT_PTR)PsGetProcessId((PEPROCESS)pOperationInformation->Object);
	...
	if (m_EventList->IsCapturing() && pOperationInformation->Object != NULL)
	{
		...
		if (EventId)
		{
			...
			if (data)
			{
				...
				data->ProcessId = (ULONG)(UINT_PTR)PsGetCurrentProcessId();
				data->ThreadId = (ULONG)(UINT_PTR)PsGetCurrentThreadId();
				data->TargetProcessId = (ULONG)(UINT_PTR)PsGetThreadProcessId((PETHREAD)pOperationInformation->Object);
				data->TargetThreadId = (ULONG)(UINT_PTR)PsGetThreadId((PETHREAD)pOperationInformation->Object);
				data->DesiredAccess = (ULONG)pOperationInformation->Parameters->CreateHandleInformation.GrantedAccess;
				data->ResultStatus = (ULONG)pOperationInformation->ReturnStatus;
